...
async function main() {
  ...
  head('Billing via Stripe (optional — skip for localhost)')
  let STRIPE_SECRET_KEY = 'sk_test_placeholder'
  let STRIPE_WEBHOOK_SECRET = 'whsec_placeholder'
  let STRIPE_MONTHLY_PRICE_ID = 'price_placeholder'
  let STRIPE_ANNUAL_PRICE_ID = 'price_placeholder'
  if (await yes('Enable Stripe billing? (most local users skip)', false)) {
    STRIPE_SECRET_KEY = await ask('STRIPE_SECRET_KEY (sk_test_...)', STRIPE_SECRET_KEY)
    STRIPE_WEBHOOK_SECRET = await ask('STRIPE_WEBHOOK_SECRET (whsec_...)', STRIPE_WEBHOOK_SECRET)
    STRIPE_MONTHLY_PRICE_ID = await ask('STRIPE_MONTHLY_PRICE_ID', STRIPE_MONTHLY_PRICE_ID)
    STRIPE_ANNUAL_PRICE_ID = await ask('STRIPE_ANNUAL_PRICE_ID', STRIPE_ANNUAL_PRICE_ID)
  } else {
    warn("    Using placeholder Stripe values so the app boots. Billing pages won't work until real keys are set.")
  }
...
# --- Stripe (billing; placeholders are fine for local) ---
